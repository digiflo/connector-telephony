...
export class VoipAgent {
    constructor(env, services) {
        ...
        this.voip = services.voip_freepbx;
    ...
    async connectAgent() {
        ...
        */
        try {
            await loadBundle("voip_freepbx.agent_assets");
        } catch (error) {
            console.error(error);
    ...
    async createCall(options) {
        ...
        // Store call directly instead of using mail.store models
        // Ensure call and partner are always objects
        this.voip.call = call || {};
        this.voip.partner = (call && call.partner) ? call.partner : {};
    }
    async call({number, partner}) {
        ...
        if (this.voip.mode === "prod") {
            ...
            this.session = new SIP.Inviter(this.agent, destination_number, {
                // Enable early media for ringback tone from remote
                earlyMedia: true,
            });
            ...
            this.session
                .invite({
                    requestDelegate: {
                        ...
                        onProgress: this._onInviteProgress.bind(this),
                    },
                })
                .catch((error) => {
    ...
    _onInviteProgress(response) {
        // Handle 180 Ringing / 183 Session Progress with early media
        // This allows the remote ringback tone to come through
        if (!this.session) {
            return;
        }
        // If we have a session description (early media), set up audio
        if (this.session.sessionDescriptionHandler) {
            this.setCallAudio();
        }
    }
    ...
    async _onHanghup() {
        ...
        if (this.voip.call && this.voip.call.id) {
            this.voip.call = await this.orm.call("voip.call", "terminate_call", [[this.voip.call.id]]);
        }
        this.voip.inCall = false;
        this.voip.partner = {};
        this.voip.call = {};
...
export const voipAgentOCAService = {
    dependencies: ["voip_freepbx", "multi_tab", "notification", "orm", "mail.store"],
    async start() {
